...
export default function OrderSuccess() {
  ...
  useEffect(() => {
    // Animate only translate/scale — never set opacity:0 so page is always visible
    const targets = [iconRef.current, cardRef.current].filter(Boolean);
    if (!targets.length) return;
    gsap.fromTo(
      targets,
      { y: 28, scale: 0.97 },
      { y: 0, scale: 1, duration: 0.55, stagger: 0.1, ease: "power3.out", clearProps: "all" }
    );
  }, []);
  ...
  return (
    <>
      <style>{`
        ...
        .pulse-a { animation: pulse-ring 1.8s ease-out infinite; }
        .pulse-b { animation: pulse-ring 1.8s ease-out 0.55s infinite; }
      ...
      >
        <div className="w-full max-w-md flex flex-col items-center gap-8">
          ...
          {/* ── Main card ── */}
          <div ref={cardRef} className="w-full bg-white rounded-3xl shadow-sm border border-[#ede7df] px-7 py-9 text-center">

            <p className="text-[9px] tracking-[0.35em] uppercase text-[#a89880] mb-2">Order Placed</p>
            <h1
            ...
            {orderId && (
              <div className="inline-flex items-center gap-2 bg-[#f5f0eb] rounded-full px-4 py-1.5 mb-5">
                <span className="text-[9px] uppercase tracking-widest text-[#a89880]">Order ID</span>
                <span className="text-[13px] font-semibold text-[#2c2c2c] tracking-wide">{orderId}</span>
              </div>
            )}

            <p className="text-[13px] text-[#7a6e65] leading-relaxed mb-6">
              Your order has been sent to our team. You've also received a copy on your WhatsApp.
              We'll send you a <span className="text-[#2c2c2c] font-medium">confirmation message</span> once it's ready.
            </p>
            ...
            <div className="flex flex-col gap-2.5 text-left mb-8">
              <Step icon="📩" label="Order Received"
                desc="Your order details have been sent to our team." done />
              <Step icon="✅" label="Confirmation"
                desc="We'll WhatsApp you to confirm your order shortly." active />
              <Step icon="🚚" label="Dispatch"
                desc="Your perfume will be packed and shipped." />
              <Step icon="🎁" label="Delivered"
                desc="Enjoy your fragrance!" />
            </div>
            ...
            <div className="flex flex-col gap-3">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-full py-3.5 rounded-full bg-green-500 hover:bg-green-600 text-white
                  text-[11px] uppercase tracking-widest font-medium flex items-center justify-center gap-2 transition-all"
              >
                <WaIcon /> Check Order Status
              </a>
              <button
                onClick={() => navigate("/")}
                className="w-full py-3.5 rounded-full border border-[#2c2c2c] text-[#2c2c2c]
                  text-[11px] uppercase tracking-widest font-medium hover:bg-[#2c2c2c] hover:text-white transition-all"
              >
                Continue Shopping
              </button>
            </div>
          ...
          <p className="text-[11px] text-[#a89880] text-center">
            Need help? Call{" "}
            <a href={`tel:${SHOP_PHONE}`} className="text-[#2c2c2c] font-medium hover:underline">
              {SHOP_PHONE}
            </a>
          </p>
  ...
  return (
    <div className={`flex items-start gap-3 px-4 py-3 rounded-xl
      ${done   ? "bg-green-50 border border-green-100" :
        active  ? "bg-[#f5f0eb] border border-[#e0d8d0]" :
                  "opacity-50"}`}
    >
      <span className="text-base mt-0.5 shrink-0">{icon}</span>
      <div>
        <p className={`text-[12px] font-medium
          ${done ? "text-green-700" : active ? "text-[#2c2c2c]" : "text-[#a89880]"}`}>
          {label}
          {done   && <span className="ml-2 text-[9px] bg-green-200 text-green-800 px-2 py-0.5 rounded-full uppercase tracking-wide">Done</span>}
          {active && <span className="ml-2 text-[9px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full uppercase tracking-wide">Pending</span>}
        </p>
        <p className="text-[11px] text-[#a89880] mt-0.5 leading-relaxed">{desc}</p>
      </div>
    </div>
  );
}
