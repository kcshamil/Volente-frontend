...
import { CarCards } from "../components/Products";
  ...
  return (
    <>
      <style>{`
        ...
        .car-pill:hover { background: #2c2c2c; border-color: #2c2c2c; color: #fff; }
        ...
        .car-stats-block:first-child { border-left: none; }
      ...
      {/* ── HERO ── */}
      <section
        className="car-hero-bg car-grain relative flex flex-col items-center justify-center text-center overflow-hidden"
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center w-full" style={{ maxWidth: 360 }}>
          ...
          >
            Car
            <br />
            <span className="italic" style={{ color: "#c9b99a" }}>Fragrances</span>
          </h1>
          ...
          >
            Elevate every journey. Premium hanging diffusers and car fragrances crafted for your vehicle — rich leather, smoked wood, and ambient freshness that redefine the drive.
          </p>
        ...
        <div className="relative z-10 w-full mt-auto" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="car-stats-grid" style={{ padding: "14px 8px" }}>
            {[
              { num: "3+",   label: "Fragrances" },
              { num: "2",    label: "Styles" },
              { num: "45d+", label: "Longevity" },
              { num: "Premium", label: "Design" },
            ].map(({ num, label }) => (
              <div key={label} className="car-stats-block">
      ...
      >
        ...
        {["All", "Signature", "Bestseller", "New", "Premium", "Fresh"].map((tag) => (
          <button key={tag} className="car-pill">{tag}</button>
        ))}
      ...
      <div className="collection-header" style={{ background: "#ede7df", padding: "28px 16px 12px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <p style={{ fontFamily: "Barlow, sans-serif", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", color: "#a89880" }}>3 Products</p>
      </div>
      ...
      {/* ── COLLECTION STORY ── */}
      <section style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e293b 100%)", padding: "60px 20px", textAlign: "center", overflow: "hidden", fontFamily: "Barlow, sans-serif" }}>
        <p style={{ fontSize: 9, letterSpacing: "0.4em", textTransform: "uppercase", color: "#c9b99a", marginBottom: 18 }}>The Driving Philosophy</p>
        <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(20px, 5.5vw, 42px)", fontWeight: 300, color: "#fff", lineHeight: 1.6, maxWidth: 520, margin: "0 auto 24px" }}>
          "Every route becomes a sensory journey."
        </h2>
        <div style={{ width: 32, height: 1, background: "#c9b99a", margin: "0 auto 28px" }} />
        <div style={{ display: "flex", flexDirection: "column", gap: 18, alignItems: "center" }}>
          {[
            { label: "Leather",  desc: "Black Leather · Tuscan Suede · Tobacco Wood" },
            { label: "Woody",    desc: "Cedarwood · Sandalwood · Warm Amber" },
            { label: "Fresh",    desc: "Fresh Lavender · White Musk · Sweet Vanilla" },
          ].map(({ label, desc }) => (
